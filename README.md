...
| `streamlink_quality` | `best` | 固定画质（`adaptive_quality: false` 时生效） |
| `adaptive_quality` | `true` | 自适应画质开关；开启时忽略 `streamlink_quality`，自动从 `streamlink_quality_ladder` 中选档 |
| `streamlink_quality_ladder` | `["best","720p","480p","worst"]` | 画质阶梯，index 0 = 最高画质；网络波动时向后降档，稳定后向前升档 |
| `quality_downgrade_threshold` | `60` | 录制时长（秒）低于此值且 ffmpeg 异常退出 → 降一档画质 |
| `quality_upgrade_threshold` | `300` | 录制时长（秒）高于此值且 ffmpeg 正常退出 → 升一档画质 |
...
| `WATERMARKING` | 录制完成，正在添加时间水印（含画质标签） |
...
当 `timestamp_watermark` 设为 `true`，后处理阶段会对每段录制用 ffmpeg 重新编码，在视频右下角烙印白色半透明水印：

```
超清  2026-03-20 02:42:45
```

- **左侧**：画质标签（超清 / 高清 / 标清 / 流畅，对应 streamlink 档位）
- **右侧**：实时时间戳（与画面内容同步，由 `basetime` + `strftime` 驱动）
- 两者以两个空格分隔，位于同一行

水印字体默认为 `NotoSansCJK-Regular.ttc`（支持中文），可通过 `watermark_font` 配置覆盖。

---

## 自适应画质

当 `adaptive_quality` 设为 `true`（默认），监控服务会根据录制稳定性自动调整 streamlink 画质档位：

| 情形 | 行为 |
|------|------|
| ffmpeg **异常退出** 且录制时长 < `quality_downgrade_threshold`（60s） | 降一档画质，下次录制使用更低档位 |
| ffmpeg **正常退出** 且录制时长 ≥ `quality_upgrade_threshold`（300s） | 升一档画质，下次录制尝试更高档位 |
| 其他情形 | 档位不变 |

画质阶梯由 `streamlink_quality_ladder` 定义，默认：

```json
["best", "720p", "480p", "worst"]
```

index 0 为最高画质；网络稳定时始终尝试 index 0；网络频繁波动时向后退档直到录制可维持。当前档位随服务状态一起持久化到磁盘（`.douyin_state_{主播名}.json`），重启后不重置。
...
模型首次运行时会自动从 HuggingFace 下载（国内网络建议配置环境变量 `HF_ENDPOINT=https://hf-mirror.com`，可在 systemd 服务文件的 `Environment=` 中设置）。
...
**Q：主机开了代理（sing-box / clash 等），录制流量会走代理吗？**
A：抖音 CDN 域名（`douyincdn.com` 等）和其解析出的 IP 均为国内地址。如果代理工具的路由规则将 `geosite-cn` / `geoip-cn` 设为直连（大多数配置的默认行为），录制流量不会经过代理节点。若不确定，可在代理面板中检查连接日志。
